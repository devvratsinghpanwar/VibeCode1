...
export function AreaChart({ data, areas, loading, emptyMessage = 'No data available', style }: AreaChartProps) {
  ...
  return (
    <div ref={chartRef} className="bg-white dark:bg-neutral-900 rounded shadow p-4 w-full" style={style}>
      <div className="flex justify-between items-center mb-2">
        <span className="font-semibold text-lg">Cumulative Data</span>
        <button
      ...
      <ResponsiveContainer width="100%" height={300}>
        <ReAreaChart data={data} margin={{ top: 16, right: 24, left: 0, bottom: 0 }}>
          ...
          {areas.map((area) => (
            <Area
              ...
              fill={area.color || `var(--color-primary)`}
              fillOpacity={0.2}
              strokeWidth={3}
              name={area.name}
